  ...
  return (
    <div className="min-h-screen bg-[#050505] text-white p-6 relative overflow-hidden">
      ...
      <nav className="max-w-md mx-auto flex items-center justify-between py-6 mb-8 border-b border-white/5 relative z-10">
        <Link href="/faculty" className="text-white/50 hover:text-purple-400 transition-colors flex items-center gap-2 font-mono text-sm bg-white/5 px-4 py-2 border border-white/10 rounded-md">
          <ArrowLeft className="w-4 h-4" /> Exit Scanner
        </Link>
        <div className="font-bold text-purple-400 text-sm font-mono tracking-widest uppercase">Classroom Scanner</div>
      </nav>
      ...
      <main className="max-w-md mx-auto space-y-8 relative z-10">
        {!result && !error && (
          <div className="space-y-6">
            ...
            <div className="bg-black border border-purple-500/30 p-2 rounded-xl shadow-[0_0_30px_rgba(168,85,247,0.1)]">
              <QRScanner onScanSuccess={handleScan} />
            </div>
            {loading && <p className="text-center text-purple-400 animate-pulse font-mono text-sm flex items-center justify-center gap-2"><Terminal className="w-4 h-4"/> Verifying...</p>}
          </div>
        )}

        {result && (
          <div className="p-8 rounded-2xl bg-white/5 border border-purple-500/30 shadow-[0_0_40px_rgba(168,85,247,0.15)] text-center space-y-6 backdrop-blur-xl">
            <CheckCircle2 className="w-16 h-16 text-purple-400 mx-auto drop-shadow-[0_0_15px_rgba(168,85,247,0.5)]" />
            <div>
              <h2 className="text-2xl font-black text-white">Pass Verified</h2>
              <p className="text-purple-400 font-bold mt-1 font-mono text-sm">Authorized for Leave</p>
            </div>
            <div className="space-y-4 text-left bg-black/50 border border-white/10 p-6 rounded-xl font-mono text-sm">
              <div>
                ...
                <p className="text-cyan-400">{result.student.roll_no} • {result.student.department}</p>
              </div>
              <div className="h-px bg-white/10 w-full" />
              <div>
                <p className="text-white/40 uppercase tracking-widest text-[10px] mb-1">Registered Event</p>
                <p className="font-bold text-purple-400">{result.event.title}</p>
                <p className="text-white/60">Venue: {result.event.venue}</p>
              </div>
            </div>
            <button 
              onClick={() => setResult(null)} 
              className="w-full py-4 bg-purple-500/20 hover:bg-purple-500/30 border border-purple-500/50 text-purple-400 rounded-xl font-bold transition-all shadow-[0_0_15px_rgba(168,85,247,0.2)]"
            >
              Scan Next Pass
            </button>
          </div>
