...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-slate-900 via-[#050505] to-black text-white selection:bg-cyan-500/30 overflow-hidden relative">
      ...
      <nav className="max-w-7xl mx-auto flex items-center justify-between py-6 border border-white/10 relative z-10 bg-white/[0.02] backdrop-blur-3xl px-8 mt-4 rounded-3xl shadow-[0_8px_32px_0_rgba(0,0,0,0.3)]">
        <div className="flex items-center gap-4">
          <div className="w-10 h-10 bg-cyan-500/10 border border-cyan-500/20 rounded-lg flex items-center justify-center text-cyan-400 font-black text-xl shadow-[0_0_15px_rgba(6,182,212,0.2)]">
            C
          </div>
          <div className="text-xl font-black tracking-tighter text-white">
            CIT PASS
            <span className="block text-[10px] text-cyan-400 font-mono tracking-widest uppercase mt-0.5">Verification Portal</span>
          </div>
        </div>
        <div className="hidden sm:flex space-x-4 items-center">
          <Link href="/login/faculty" className="px-5 py-2.5 rounded-xl bg-purple-500/10 backdrop-blur-md border border-purple-500/20 text-purple-400 hover:bg-purple-500 hover:text-white hover:shadow-[0_0_20px_rgba(168,85,247,0.4)] transition-all text-sm font-bold tracking-wide">
            Faculty Login
          </Link>
          ...
          <Link href="/login/student" className="px-5 py-2.5 rounded-xl bg-cyan-500/10 backdrop-blur-md border border-cyan-500/20 text-cyan-400 hover:bg-cyan-400 hover:text-black hover:shadow-[0_0_20px_rgba(6,182,212,0.4)] transition-all text-sm font-bold tracking-wide">
            Student Login
          </Link>
        </div>
      </nav>
        ...
        <div className="grid lg:grid-cols-2 gap-16 items-center">
          ...
          >
            ...
            <motion.div variants={itemVariants} className="flex flex-col sm:flex-row items-center gap-4 pt-6 relative z-10">
              <Link href="/login/student" className="group flex items-center justify-center gap-2 w-full sm:w-auto px-8 py-4 bg-gradient-to-r from-cyan-500/80 to-blue-600/80 backdrop-blur-xl text-white rounded-xl font-bold hover:scale-105 transition-all shadow-[0_0_30px_rgba(6,182,212,0.4)] border border-cyan-400/50">
              ...
              <Link href="/coordinator/import" className="group flex items-center justify-center gap-2 w-full sm:w-auto px-8 py-4 bg-white/5 text-white border border-white/10 rounded-xl font-bold hover:bg-white/10 transition-all backdrop-blur-xl shadow-[0_8px_32px_0_rgba(0,0,0,0.2)]">
                <Terminal className="w-5 h-5 text-white/40 group-hover:text-cyan-400 transition-colors" /> Upload Registrations
              </Link>
            </motion.div>
          </motion.div>
